  ...
  const submit = async () => {
    ...
    try {
      ...
      if (isBlocked(e)) { setErr("Adblock detectado — desactívalo para este sitio y recarga."); }
        ...
        const m = { "auth/email-already-in-use": "Email ya registrado", "auth/wrong-password": "Contraseña incorrecta", "auth/user-not-found": "Usuario no encontrado", "auth/weak-password": "Mínimo 6 caracteres", "auth/invalid-email": "Email inválido", "auth/invalid-credential": "Email o contraseña incorrectos" };
      ...
      <div style={S.center}>
        <div style={{ width: "100%", maxWidth: "360px" }}>
          <div style={{ marginBottom: "32px" }}>
            <div style={{ fontSize: "24px", fontWeight: 700, letterSpacing: "-0.5px" }}>MisGastos</div>
            <div style={{ color: C.muted, fontSize: "14px", marginTop: "4px" }}>Gastos compartidos en tiempo real</div>
          </div>
          ...
          {mode === "register" && <Input label="Nombre" placeholder="Miguel" value={name} onChange={e => setName(e.target.value)} />}
          <Input label="Email" type="email" placeholder="tu@email.com" value={email} onChange={e => setEmail(e.target.value)} autoCapitalize="none" />
          <Input label="Contraseña" type="password" placeholder="••••••••" value={pass} onChange={e => setPass(e.target.value)} onKeyDown={e => e.key === "Enter" && submit()} />
...
function Dashboard({ user, group, onBack }) {
  ...
  useEffect(() => {
    return onSnapshot(
      query(collection(db, "groups", group.id, "expenses"), orderBy("createdAt", "desc")),
    ...
    <div style={S.page}>
      <div style={S.wrap}>
        ...
        <div style={S.header}>
          ...
          <button style={S.btnSm} onClick={onBack}>← Grupos</button>
        </div>
        ...
        <div style={{ background: C.black, borderRadius: "14px", padding: "20px", marginBottom: "20px" }}>
          ...
          <div style={{ display: "flex", gap: "6px", marginTop: "14px", flexWrap: "wrap" }}>
            {memberList.map(([uid, name]) => {
              ...
              return (
                <div key={uid} style={{ background: "rgba(255,255,255,0.1)", borderRadius: "8px", padding: "5px 10px", fontSize: "12px", color: C.white }}>
                  {name.split(" ")[0]} <span style={{ color: bal >= 0 ? "#4ade80" : "#f87171", fontWeight: 700 }}>{bal >= 0 ? "+" : ""}{fmt(bal)}</span>
                </div>
              );
            })}
          </div>
                ...
                <div key={e.id} style={S.card} onClick={() => setShowDetail(e)}>
                  <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                    ...
                    <div style={{ textAlign: "right" }}>
                      ...
                      <div style={{ color: C.muted, fontSize: "11px" }}>{fmt(e.share || 0)}/p.</div>
                    </div>
                  </div>
                </div>
              );
            ...
            {debts().length === 0 ? (
              ...
              <div key={i} style={{ ...S.card, cursor: "default" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "14px" }}>
                  ...
                  <span style={{ marginLeft: "auto", fontWeight: 700, fontSize: "16px" }}>{fmt(d.amount)}</span>
                </div>
              </div>
            ))}
            <div style={S.divider} />
      ...
      {showAdd && (
        <div style={S.overlay} onClick={e => e.target === e.currentTarget && setShowAdd(false)}>
          <div style={S.sheet}>
            <div style={S.handle} />
            <div style={{ fontWeight: 700, fontSize: "17px", marginBottom: "20px" }}>Nuevo gasto</div>
        ...
        <div style={S.overlay} onClick={e => e.target === e.currentTarget && setShowDetail(null)}>
          <div style={S.sheet}>
            ...
            {(showDetail.paidBy === user.uid || group.createdBy === user.uid) && (
              <button style={{ ...S.btn, background: C.red }} onClick={() => deleteExpense(showDetail.id)}>Eliminar gasto</button>
            )}
          </div>
        </div>
      )}
