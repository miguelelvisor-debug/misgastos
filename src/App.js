...
function Dashboard({ user, group, onBack }) {
  ...
  useEffect(() => {
    return onSnapshot(
      query(collection(db, "groups", group.id, "settlements"), orderBy("createdAt", "desc")),
  ...
  return (
    <div style={S.page}>
      ...
      {/* ADD / EDIT EXPENSE */}
      {showAdd && (
        <div style={S.overlay} onClick={e => { if (e.target === e.currentTarget) { setShowAdd(false); setEditingExpense(null); setDesc(""); setAmount(""); setCat("super"); setPaidBy(user.uid); } }}>
          <div style={S.sheet}>
            <div style={S.handle} />
