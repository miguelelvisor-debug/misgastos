...
import {
  collection, doc, setDoc, getDoc, addDoc, getDocs, onSnapshot,
...
function GroupScreen({ user, onGroup }) {
  ...
  const joinGroup = async () => {
    ...
    try {
      ...
      setShowJoin(false);
      setCode("");
      onGroup({ id: gDoc.id, ...gData, members: [...(gData.members || []), user.uid], memberNames: { ...(gData.memberNames || {}), [user.uid]: user.displayName } });
    } catch (e) {
      setErr("Error al unirse. Comprueba el código 😕");
      console.error(e);
    }
  };
